...
${JSON.stringify(jwtString)}`}
            ...
            <SyntaxHighlighter wrapLongLines language="json" >
              {JSON.stringify(decodedJwt, null, 2) !== undefined ?`// JWT Payload
${JSON.stringify(decodedJwt, null, 2)}` : "Not Available"}
            </SyntaxHighlighter>
            <Stack
              spacing={1}
              sx={{
            ...
            >
              ...
              <Typography>
                <code>sub (subject)</code>：<b>Subject (user identifier, unique for each user)</b>
              </Typography>
              <Typography>
                <code>nonce</code>：Signature order (values generated by assembling URL parameters earlier)
              </Typography>
              <Typography>
